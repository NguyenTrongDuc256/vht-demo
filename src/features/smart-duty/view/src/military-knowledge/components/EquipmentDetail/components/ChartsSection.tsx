...
import type {
  DashboardChartPoint,
  DashboardLocationFrequency,
} from '../../../constants/equipment-dashboard.mock';
import { DashboardCard } from './DashboardCard';
import { equipmentColors } from '../theme';

interface ChartsSectionProps {
  monthlyTrend: DashboardChartPoint[];
  locationFrequency: DashboardLocationFrequency[];
}
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1.5, mt: 1.5 }}>
        ...
        >
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={monthlyTrend} margin={{ top: 8, right: 16, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: equipmentColors.card, border: `1px solid ${equipmentColors.cardBorder}`, borderRadius: 4, fontSize: 11 }} />
      ...
      <Box sx={{ flex: 1 }}>
        ...
        >
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={locationFrequency} layout="vertical" margin={{ top: 4, right: 16, left: 4, bottom: 0 }}>
              ...
              <YAxis type="category" dataKey="location" width={72} tick={{ fill: equipmentColors.textSecondary, fontSize: 9 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: equipmentColors.card, border: `1px solid ${equipmentColors.cardBorder}`, borderRadius: 4, fontSize: 11 }} />
              <defs>
              ...
              <Bar dataKey="count" radius={[0, 4, 4, 0]} fill="url(#barGrad)" />
            </BarChart>
          </ResponsiveContainer>
        </DashboardCard>
      </Box>
